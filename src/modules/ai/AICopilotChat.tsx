...
export function AICopilotChat({ data }: { data: ChatData }) {
  ...
  const handlePromptClick = (prompt: string) => {
    ...
    const response = data.promptResponses?.[prompt];
    
    if (response) {
      const aiMsg: Message = { sender: "ai", text: response };
      setMessages(prev => [...prev, userMsg, aiMsg]);
    } else {
      const aiMsg: Message = { sender: "ai", text: `I'll analyze your "${prompt}" request. Generating insights from your latest ESG data...` };
      setMessages(prev => [...prev, userMsg, aiMsg]);
    }
  };
  ...
  const handleSend = () => {
    ...
    const aiMsg: Message = { sender: "ai", text: `I've received your query: "${inputValue}". Based on your current ESG data, I'm analyzing the relevant metrics across Environmental, Social, and Governance dimensions. Your overall ESG posture remains strong with key improvements in carbon intensity this quarter.` };
    setMessages(prev => [...prev, userMsg, aiMsg]);
    setInputValue("");
  };
        ...
        <div className="max-w-4xl w-full mt-xl space-y-lg flex flex-col">
          {messages.map((msg, i) => (
            <div key={i} className={`p-lg rounded-[24px] max-w-3xl shadow-sm ${msg.sender === 'ai' ? 'glass-panel rounded-tl-none self-start' : 'bg-primary text-on-primary rounded-br-none self-end max-w-xl'}`}>
              <p className="font-body-md whitespace-pre-line">{msg.text}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-xl z-10 flex flex-col items-center gap-lg">
        ...
        <div className="flex gap-md overflow-x-auto max-w-4xl no-scrollbar pb-sm">
          {data.suggestedPrompts.map((prompt, i) => (
            <button 
              ...
              className="whitespace-nowrap px-lg py-md rounded-full border border-outline-variant bg-white/60 hover:bg-white hover:border-primary/50 text-body-sm font-medium transition-all active:scale-95 shadow-sm"
            >
              {prompt}
            </button>
          ))}
        ...
        <div className="w-full max-w-4xl relative">
          <div className="glass-panel p-sm rounded-[28px] flex items-center gap-sm shadow-xl active-glow ring-2 ring-primary/5">
            ...
            <input 
              className="flex-1 bg-transparent border-none focus:ring-0 text-body-md py-md px-xs outline-none" 
            ...
            <button 
              onClick={handleSend}
              className="w-12 h-12 flex items-center justify-center bg-primary text-on-primary rounded-full hover:opacity-90 shadow-lg shadow-primary/20 transition-all"
            >
