    ...
    <div className="col-span-12 lg:col-span-4 bg-primary text-on-primary rounded-xl p-lg flex flex-col shadow-lg relative overflow-hidden">
        ...
        {items.map((item, i) => (
          <div key={i} className="p-md bg-white/10 rounded-lg flex items-center justify-between border border-white/10 hover:bg-white/20 transition-colors cursor-pointer group">
            <div className="flex flex-col">
              <span className="font-body-md font-medium">{item.title}</span>
              <span className="text-[11px] font-label-md text-on-primary-container">Submission ID: {item.id}</span>
            </div>
      ...
      <div className="mt-auto pt-lg">
        <div className="flex justify-between items-end">
          ...
          <button className="bg-secondary-fixed text-on-secondary-fixed font-bold px-lg py-sm rounded-full text-label-md shadow-lg hover:scale-105 active:scale-95 transition-all">
            Batch Approve
          </button>
        </div>
      </div>
